...
environment { 
    ...
    stages {
        ...
        stage('BUILD IMAGE') {
			      agent{label 'docker'}
            steps {
                sh 'docker build -t $registry:$dockerTag_$BUILD_NUMBER .'             
            }
        }
        stage('PUSH HUB') { 
			      agent{label 'docker'}
            steps {
                script {
                    docker.withRegistry( '', '$registryCredential' ) {
                        sh 'docker tag $registry:$dockerTag_$BUILD_NUMBER $registry:$dockerTag_$BUILD_NUMBER'
                        sh 'docker push $registry:$dockerTag_$BUILD_NUMBER'
                    }
                }    
            }
        }
